...
public class SetGameStrings {
  ...
  public String generateWinnerString(String first, String last) {
     public final String[] winnerStrings = {
        String.format("The name is %s. %s %s.", last, first, last),
        String.format("If you want to crown %s, then crown zer ass", first, last[0]),
        String.format("All %s %s does is win", first, last[0]),
        "Fuck ya",
    ...
    int index = Math.random() * winnerStrings.length;
    return winnerStrings[index];
  }

  public String generateLoserString(String first, String last) {
    public final String[] loserStrings = {
        ":(",
        ...
        "Anirwin."};
    int index = Math.random() * loserStrings.length;
